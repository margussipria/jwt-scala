package eu.sipria.jwt.exceptions

import eu.sipria.jwt.JwtTime

class JwtException(message: String) extends RuntimeException(message)
...
class JwtNonSupportedAlgorithm(alg: String) extends JwtException(s"The algorithm [$alg] is not currently supported.")
