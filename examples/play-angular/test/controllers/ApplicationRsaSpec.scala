...
class ApplicationRsaSpec extends PlaySpec with OneAppPerSuite with Results {

  implicit override lazy val app: play.api.Application = FakeApplication(additionalConfiguration = Map(
    "eu.sipria.play.jwt.algorithm" -> "RS512",
    "eu.sipria.play.jwt.key.type" -> "RSA",
    "eu.sipria.play.jwt.key.base64" -> "true",
    "eu.sipria.play.jwt.key.rsa.public" -> "conf/rsa.public.pem",
    "eu.sipria.play.jwt.key.rsa.private" -> "conf/rsa.private.pem"
  ))
